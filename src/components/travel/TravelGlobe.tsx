...
const STOPS: PositionedStop[] = [
  { name: "Taiwan", heading: "Taiwan", detail: "Cherry blossoms, tiny alleyways, and too much matcha.", flag: "🇯🇵" },
  { name: "Japan", heading: "Japan", detail: "Cherry blossoms, tiny alleyways, and too much matcha.", flag: "🇯🇵" },
  { name: "Hawaiʻi", heading: "Maui", detail: "Salt in my hair and a very happy camera roll.", flag: "🌺" },
  { name: "China", heading: "China", detail: "A homecoming full of family, food, and familiar streets.", flag: "🇨🇳" },
...
export default function TravelGlobe() {
  ...
  const angleRef = useRef(STOPS[1].angle);
  const isDraggingRef = useRef(false);
  const dragStartRef = useRef<null | { clientX: number; angle: number }>(null);
  const snapAnimationRef = useRef<number | null>(null);
  const [angle, setAngle] = useState(STOPS[1].angle);
  const [activeStop, setActiveStop] = useState<PositionedStop>(STOPS[1]);
  ...
  const guideStart = pointAt(MIN_ANGLE);
  const guideEnd = pointAt(MAX_ANGLE);

  return (
    <main className={styles.page}>
      <h1 className={styles.stopHeading} aria-live="polite">{activeStop.heading}</h1>
      <section className={styles.globeStage} aria-label="Interactive travel globe">
        <svg
          ref={svgRef}
        ...
        >
          ...
          {STOPS.map((stop) => {
            const location = pointAt(stop.angle, radius + 3);
            ...
            return (
              ...
              >
                <circle cx={location.x} cy={location.y} r="34" className={styles.stopHitArea} />
                <circle cx={location.x} cy={location.y} r={selected ? 12 : 8} className={selected ? styles.stopActive : styles.stopDot} />
              </g>
            );
          })}
          ...
          >
            ...
            <g transform={`rotate(${planeRotation})`} filter="url(#soft-shadow)" className={styles.plane}>
              <path d="M0-29 7-7 42 8v9L7 11 0 31l-7-20-42 6V8l35-15Z" />
              <path d="M0-24v47" />
            </g>
          </g>
        </svg>
